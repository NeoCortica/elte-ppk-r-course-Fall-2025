...
```{r}
#Megnyitjuk a szükséges packegeket
...
library(janitor)
library(readxl)
#Beolvassuk az excelt és megszabadulunk a rosszul formatált nevektől (janitor::clean_names)
df <- read_xlsx("/Users/zsoltternei/assignment/elte-ppk-r-course-Fall-2025/data/assignment_3_dataset.xlsx") %>%
  clean_names()
```
...
#Rápillantunk
glimpse(df)
#Gyorsan összegezzük hogy miből is áll ez a sok col, néhány furcsa értéket máris láthatunk, pl pain 50, sex 3 különböző, és nem other, mert végigfutottam View()-al. Többi érték szerencsére megfelel.
skim(df)
df%>%distinct(sex) #woman. 
...
```{r}
simple_model <- lm(pain ~ age + sex, data = df)
```
